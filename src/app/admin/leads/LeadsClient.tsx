...
export default function LeadsClient() {
  ...
  const handleExport = async () => {
    try {
      ...
      const BOM = "\uFEFF";
      const csv = BOM + "Name,Email,Phone,Company,Message,Date,Read\n" +
  ...
  return (
    <div className="space-y-6">
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-border">
                  {leads.map((lead) => (
                    <tr
                      key={lead.id}
                      className={`hover:bg-muted/30 transition-colors ${!lead.is_read ? "bg-accent/10 font-medium text-foreground" : "text-muted-foreground"
                        } cursor-pointer`}
                    ...
                    >
                      ...
                      <td className="py-4 px-4 whitespace-nowrap">
                        <Badge variant="outline" className="font-medium text-[10px] text-foreground">
                      ...
                      return pages.map((p, i) => (
                        p === '...' ? (
                          ...
                          <Button
                            ...
                            className={`h-8 w-8 p-0 text-[10px] rounded-md transition-all text-foreground ${page === p ? "bg-accent text-accent-foreground shadow-sm" : ""}`}
                            onClick={() => {
                              fetchLeads(Number(p));
                            }}
                          >
      ...
      <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
        <DialogContent className="max-w-xl p-0 overflow-hidden border-none shadow-2xl [&>button]:text-white [&>button]:opacity-100 [&>button]:hover:text-white/80 transition-all bg-white">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-bold text-foreground flex items-center gap-1.5 underline decoration-accent/30 decoration-2 break-all">
              ...
              <div className="flex justify-end gap-3 pt-4">
                <Button variant="outline" onClick={() => setDetailsOpen(false)} className="rounded-xl px-6 text-foreground">Close</Button>
                <Button
                  variant="ghost"
