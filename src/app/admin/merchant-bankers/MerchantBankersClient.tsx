...
export default function MerchantBankersClient() {
  ...
  const [smeSubcategories, setSmeSubcategories] = useState<{slug: string, name: string}[]>([]);
  ...
  useEffect(() => {
    fetchData();
    fetchBankerOptions();
    fetchSmeSubcategories();
  }, [page, limit, debouncedSearch]);

  const handleStrictNumber = (field: string, value: string, allowDecimal = false) => {
    const regex = allowDecimal ? /[^0-9.]/ : /\D/;
    if (regex.test(value)) {
      toast.error("Only numeric values are allowed");
    }
    const cleaned = allowDecimal ? value.replace(/[^0-9.]/g, "") : value.replace(/\D/g, "");
    if (allowDecimal && cleaned.split('.').length > 2) return;
  ...
  const startEdit = (b: Banker) => {
    setForm({
      ...
      mcat_id: b.mcat_id || 'list-of-sme-merchant-bankers', image: b.image || "", description: b.description || "",
            ...
            onClick={() => { 
              ...
              setForm({
                ...
                sme_ipos_by_size: JSON.stringify([
                  { title: "Very Small (< 5 Cr)", size: "" },
                  { title: "Small (5-10 Cr)", size: "" },
                  { title: "Medium (10-25 Cr)", size: "" },
                  { title: "Large (25-50 Cr)", size: "" },
                  { title: "Very Large (>50 Cr)", size: "" }
                ]),
                sme_ipos_by_subscription: JSON.stringify([
                  { title: "Subscribed upto 5 times", subscription: "" },
                  { title: "Subscribed 5 to 25 times", subscription: "" },
                  { title: "Subscribed 25 to 50 times", subscription: "" },
                  { title: "Subscribed 50 to 100 times", subscription: "" },
                  { title: "Subscribed more than 100 times", subscription: "" }
                ])
              }); 
            }}
          >
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xl relative overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <Tabs defaultValue="general" className="w-full">
            <TabsList className="flex flex-wrap h-auto gap-2 bg-transparent justify-start border-b border-border mb-6 pb-3">
              <TabsTrigger value="general" className="data-[state=active]:bg-primary/10 data-[state=active]:text-primary transition-all text-foreground">General Details</TabsTrigger>
              <TabsTrigger value="stats" className="data-[state=active]:bg-primary/10 data-[state=active]:text-primary transition-all text-foreground">Statistics</TabsTrigger>
              <TabsTrigger value="analytics" className="data-[state=active]:bg-primary/10 data-[state=active]:text-primary transition-all border-dashed border border-transparent data-[state=active]:border-primary/20 bg-secondary/30 text-foreground">Analytics Arrays</TabsTrigger>
              <TabsTrigger value="contact" className="data-[state=active]:bg-primary/10 data-[state=active]:text-primary transition-all text-foreground">Contact Info</TabsTrigger>
              <TabsTrigger value="seo" className="data-[state=active]:bg-primary/10 data-[state=active]:text-primary transition-all text-foreground">SEO & Meta</TabsTrigger>
            </TabsList>

            <TabsContent value="general" className="space-y-4 animate-in fade-in duration-300">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  <label className="text-sm font-semibold text-foreground">Banker Title/Name *</label>
                  <Input placeholder="Enter title..." value={form.title} onChange={(e) => handleTitleChange(e.target.value)} className="shadow-sm text-foreground" />
                </div>
                
                <div className="space-y-3">
                  <label className="text-sm font-semibold text-foreground">Subcategory *</label>
                  <select
                    value={form.mcat_id || "list-of-sme-merchant-bankers"}
                  ...
                  >
                    <option value="list-of-sme-merchant-bankers">Default (List of SME Merchant Bankers)</option>
                    {smeSubcategories.map(cat => (
                ...
                <div className="space-y-3 md:col-span-2">
                  <label className="text-sm font-semibold text-foreground">Description (HTML supported)</label>
                  <RichEditor
                ...
                <div className="space-y-3 md:col-span-2">
                  <label className="text-sm font-semibold text-foreground">Banker Logo</label>
            ...
            <TabsContent value="stats" className="space-y-4 animate-in fade-in duration-300">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-3"><label className="text-sm font-semibold text-foreground">Total IPOs Managed</label><Input placeholder="e.g. 50" value={form.noOfiposofar} onChange={(e) => handleStrictNumber("noOfiposofar", e.target.value)} className="shadow-sm text-foreground" /></div>
                <div className="space-y-3"><label className="text-sm font-semibold text-foreground">Total Fund Raised</label><Input placeholder="e.g. 5,000" value={formatIndianNumber(form.totalfundraised)} onChange={(e) => handleStrictNumber("totalfundraised", e.target.value, true)} className="shadow-sm text-foreground" /></div>
                <div className="space-y-3"><label className="text-sm font-semibold text-foreground">Avg IPO Size</label><Input placeholder="e.g. 100" value={formatIndianNumber(form.avgiposize)} onChange={(e) => handleStrictNumber("avgiposize", e.target.value, true)} className="shadow-sm text-foreground" /></div>
                
                <div className="space-y-3"><label className="text-sm font-semibold text-foreground">Avg Subscription</label><Input placeholder="e.g. 45" value={form.avgsubscription} onChange={(e) => handleStrictNumber("avgsubscription", e.target.value, true)} className="shadow-sm text-foreground" /></div>
                <div className="space-y-3"><label className="text-sm font-semibold text-foreground">NSE Emerge count</label><Input placeholder="0" value={form.nseemer} onChange={(e) => handleStrictNumber("nseemer", e.target.value)} className="shadow-sm text-foreground" /></div>
                <div className="space-y-3"><label className="text-sm font-semibold text-foreground">BSE SME count</label><Input placeholder="0" value={form.bsesme} onChange={(e) => handleStrictNumber("bsesme", e.target.value)} className="shadow-sm text-foreground" /></div>
              </div>
            </TabsContent>
            ...
            <TabsContent value="analytics" className="space-y-6 animate-in fade-in duration-300">
              <div className="bg-accent/5 p-4 rounded-xl border border-accent/20 mb-4">
                <p className="text-sm text-foreground/80 font-medium leading-relaxed">
                  <span className="font-bold text-primary">Dynamic Row Builders:</span> Row builders automatically compile data values for frontend visualization charts.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-3">
                  <label className="text-sm font-semibold text-foreground">SME IPOs by Size</label>
                  <JsonArrayEditor
                ...
                <div className="space-y-3">
                  <label className="text-sm font-semibold text-foreground">IPOs by Subscription Level</label>
                  <JsonArrayEditor
                ...
                <div className="space-y-3">
                  <label className="text-sm font-semibold text-foreground">Frequently Asked Questions</label>
                  <JsonArrayEditor
                    value={form.faqs}
            ...
            <TabsContent value="contact" className="space-y-4 animate-in fade-in duration-300">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3"><label className="text-sm font-semibold text-foreground">Associated Email Address</label><Input type="email" placeholder="contact@example.com" value={form.cemail} onChange={(e) => setForm({ ...form, cemail: e.target.value })} className="shadow-sm text-foreground" /></div>
                <div className="space-y-3">
                  <label className="text-sm font-semibold text-foreground">Support Mobile/Phone Number</label>
                  <Input
                ...
                <div className="space-y-3"><label className="text-sm font-semibold text-foreground">Official Website Link</label><Input placeholder="https://..." value={form.cweblink} onChange={(e) => setForm({ ...form, cweblink: e.target.value })} className="shadow-sm text-foreground" /></div>
                <div className="space-y-3 md:col-span-2"><label className="text-sm font-semibold text-foreground">Registered Office Address</label><Textarea placeholder="123 Financial District..." value={form.caddress} onChange={(e) => setForm({ ...form, caddress: e.target.value })} rows={3} className="shadow-sm text-foreground" /></div>
              </div>
            </TabsContent>
            ...
            <TabsContent value="seo" className="space-y-4 animate-in fade-in duration-300">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <Input 
                    placeholder="e.g. abc-capital-limited" 
                    value={form.slug} 
                ...
                <div className="space-y-3"><label className="text-sm font-semibold text-foreground">Meta Target Title</label><Input placeholder="Title for SEO..." value={form.meta_title} onChange={(e) => setForm({ ...form, meta_title: e.target.value })} className="shadow-sm text-foreground" /></div>
                <div className="space-y-3"><label className="text-sm font-semibold text-foreground">Meta Exact Keywords</label><Input placeholder="keywords, comma, separated" value={form.meta_keywords} onChange={(e) => setForm({ ...form, meta_keywords: e.target.value })} className="shadow-sm text-foreground" /></div>
                <div className="space-y-3 md:col-span-2"><label className="text-sm font-semibold text-foreground">Meta Description Node</label><Textarea placeholder="Description snippet for search engines..." value={form.meta_desc} onChange={(e) => setForm({ ...form, meta_desc: e.target.value })} rows={2} className="shadow-sm text-foreground" /></div>
              </div>
            </TabsContent>
          </Tabs>

          <div className="flex gap-3 pt-6 border-t border-border mt-6">
            <Button onClick={handleSave} className="bg-primary text-primary-foreground min-w-[200px] font-bold shadow-md h-11 text-base transition-transform active:scale-95">
              {editId ? "Save Banker Updates" : "Commit to Database"}
            </Button>
            <Button variant="outline" className="h-11 shadow-sm text-foreground" onClick={() => { setShowForm(false); setEditId(null); setForm(emptyForm); }}>
              Cancel Operations
            </Button>
          </div>
        </div>
              ...
              {bankers.map((b) => (
                <div key={b.id} className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:bg-secondary/10 transition-colors group">
                  ...
                  <div className="flex items-center gap-2 shrink-0 sm:opacity-70 group-hover:opacity-100 transition-opacity w-full sm:w-auto mt-3 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-0 border-border">
                    <Button variant="outline" size="sm" className="h-9 flex-1 sm:flex-auto border-border hover:bg-primary hover:text-primary-foreground hover:border-primary text-foreground" onClick={() => startEdit(b)}>
