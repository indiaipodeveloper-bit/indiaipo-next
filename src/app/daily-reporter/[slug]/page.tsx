...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!blog) {
    return {
      ...
      description: "The requested daily report blog could not be found.",
    };
  }
  ...
  const metaTitle = stripHtml(blog.meta_title) || `${stripHtml(blog.title)} - Daily IPO Reporter - India IPO`;
  const metaDescription = stripHtml(blog.description) || `Read daily report details and updates: ${stripHtml(blog.title)}`;
  const metaKeywords = blog.keyword || "Daily IPO, Daily Reporter, India IPO, stock market";
  ...
  return {
    ...
    alternates: {
      canonical: `${siteUrl}/daily-reporter/${blog.new_slug || blog.slug}`,
    },
    openGraph: {
      title: metaTitle,
      description: metaDescription,
      url: `${siteUrl}/daily-reporter/${blog.new_slug || blog.slug}`,
      type: "article",
      images: [
        {
          url: resolvedImage,
...
export default async function Page({ params }: Props) {
  ...
  } else if (category !== "daily_reporter" && category !== "Daily Reporter") {
    redirect("/daily-ipo-digest");
  }
  ...
  const articleSchema = {
    ...
    description: stripHtml(
      blog.description || `Read daily report details and updates about ${blog.title}`,
    ),
    datePublished:
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `${siteUrl}/daily-reporter/${blog.new_slug || blog.slug}`,
    },
    keywords: blog.keyword || "Daily IPO, Daily Reporter, India IPO, stock market",
    articleSection: "Daily Reporter",
    inLanguage: "en-IN",
  };
