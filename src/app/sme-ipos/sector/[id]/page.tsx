...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = activeSectorNamesStr
    ? `${activeSectorNamesStr} Sectors - SME IPOs | India IPO`
    : `SME IPOs by Sector | India IPO`;
  const description = activeSectorNamesStr
    ? `Explore the latest SME IPOs in the ${activeSectorNamesStr} sectors. Get detailed listing dates, issue sizes, GMP trends, and valuation metrics.`
    : `View SME IPOs categorized by industry sectors. Get detailed insights into company size and listing years.`;

  return {
    title,
    description,
    alternates: {
      canonical: `${BASE_URL}/sme-ipos/sector/${id}`,
    },
    openGraph: {
      title,
      description,
      url: `${BASE_URL}/sme-ipos/sector/${id}`,
      type: "website",
    }
  };
}

export default async function SmeSectorIdPage({ params }: Props) {
  const { id } = await params;
  const data = await getInitialData(id);
