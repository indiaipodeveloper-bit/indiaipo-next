...
import FlipBookWrapper from "./FlipperWrapper";
...
export default function PdfFlipper({ url }: PdfFlipperProps) {
  ...
  const [searchPageNum, setsearchPageNum] = useState<string>("");

  const scaleref = useRef<HTMLDivElement | null>(null);
  const renderingQueue = useRef<Set<number>>(new Set());
  const isInitialLoad = useRef<boolean>(true);
  const pageRefs = useRef<Map<number, string>>(new Map());
      ...
      <div className="py-4">
        <div className="flex justify-between items-center px-5 mb-4 gap-2">
          ...
          <div className="flex items-center w-full mr-10 justify-end  gap-2">
            <button
              onClick={zoomOut}
              className="flex text-2xl h-[40px] w-[40px] justify-center items-center rounded-full  cursor-pointer bg-[#3661fd] text-white"
            >
            ...
            <button
              onClick={zoomIn}
              className="flex text-xl h-[40px] w-[40px] justify-center items-center rounded-full  cursor-pointer bg-[#3661fd] text-white"
            >
        ...
        <div
          ...
          style={{
            ...
            paddingBottom: scale > 1 ? `${(scale - 1) * 700}px` : 0,
          }}
